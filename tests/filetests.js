  ...
  describe(`Tests for ${f}`, () => {
    ...
    describe('Tests on code', () => {
      ...
      it('should render as valid js code', () => {
        file = require(path.join(__dirname, '..', 'src/blocks', f))
      })
      ...
      if (fs.readFileSync(path.join(__dirname, '..', 'src/blocks', f)).toString().split('\n')[0] === '//WEBEDITOROVERRIDE') {
        describe('Web UI file tests:', () => {
          let meta
          it('should contain parsable metadata', () => {
            const rawMeta = fs.readFileSync(path.join(__dirname, '..', 'src/blocks', f)).toString().split('\n')[1].split('//meta:')[1]
